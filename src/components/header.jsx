...
const Header = () => {
    ...
    useEffect(() => {
        const fetchLive = async () => {
            try {
                ...
                setLiveMatches(all.filter(g => g.status === 'live'));
            } catch (err) {
    ...
    const navItems = [
        ...
        { name: 'LIVE',          path: '/live',          icon: 'bx-video-recording', badge: liveMatches.length > 0 ? 'LIVE' : null },
    ...
    return (
        <header className={`sticky top-0 z-50 transition-all duration-300 ${
            scrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-2' : 'bg-white shadow-sm py-3'
        }`}>
                            ...
                            <div className="pt-4 pb-3 space-y-1 border-t border-gray-100 mt-3">
                                {navItems.map((item) => (
                                    ...
                                    >
                                        ...
                                        {item.name}
                                        {item.badge && (
                                            <span className="ml-auto bg-rose-600 text-white text-[10px] px-2 py-0.5 rounded-full animate-pulse">
                                                {item.badge}
                                            </span>
                                        )}
                                    </Link>
                                ...
                                <div className="border-t border-gray-100 my-2 pt-2">
                                    <div className="px-3 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Admin</div>
                                    {adminItems.map((item) => (
                                        <Link key={item.path} to={item.path}
            ...
            {/* ── Live Indicator Bar — shows ONLY when matches are live ── */}
            <AnimatePresence>
                {liveMatches.length > 0 && (
                    <motion.div
                        ...
                        exit={{ y: -40, opacity: 0 }}
                        className="bg-gradient-to-r from-rose-600 to-red-600 text-white text-sm py-1.5"
                    >
                        <div className="container mx-auto px-4 flex items-center justify-between">
                            <div className="flex items-center gap-2 overflow-hidden">
                                <span className="w-2 h-2 bg-white rounded-full animate-pulse flex-shrink-0"></span>
                                <span className="font-bold flex-shrink-0">LIVE NOW:</span>
                                <span className="truncate text-white/90">
                                    {liveMatches.map(m => `${m.teamA} vs ${m.teamB} (${m.sport})`).join(' • ')}
                                </span>
                            </div>
                            <Link to="/live" className="text-white hover:text-white/80 underline text-xs flex-shrink-0 ml-4 flex items-center gap-1">
